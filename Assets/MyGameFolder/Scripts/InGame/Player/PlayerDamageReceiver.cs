...
namespace InGame.Player
{
    ...
    public class PlayerDamageReceiver : MonoBehaviour
    {
        ...
        [SerializeField] private string m_DamageSourceTag = "Enemy";
        ...
        private PlayerHealthModule m_HealthModule;
        private PlayerController m_PlayerController;

        private void Awake()
        {
            m_HealthModule = GetComponentInParent<PlayerHealthModule>();
            if (m_HealthModule == null)
            {
                m_HealthModule = GetComponent<PlayerHealthModule>();
            }
        ...
        private void TryApplyDamage(GameObject source, Collision2D collision)
        {
            ...
            if (grabbable != null && m_PlayerController != null)
            {
                // 現在掴んでいる場合は無視
                if (grabbable.IsGrabbed && grabbable.CurrentGrabber?.GrabberObject == m_PlayerController.gameObject)
                {
                    return;
                }
            }
            ...
            // ダメージ量を取得（IDamageSourceがあればそこから、なければデフォルト値）
